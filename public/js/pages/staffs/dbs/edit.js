$(document).ready(function() {
  ...
  $(`select.select2[name=validPoliceRecordsOption],
    ...
    select.select2[name=apctnStatus],
    select.select2[name=apctnPaidBy],
    select.select2[name=updateServiceStatus],
    select.select2[name=validType]
    `).select2({
  ...
  $("select.select2[name=dbsType]").change(function() {
    var type = $(this).val();
    switch (type) {
      case '1':
        $(".alpha").removeClass('hidden');
        $(".beta,.gamma").addClass('hidden');
