  ...
  <div class="box-body">
    ...
    <div class="pull-right m-r-10 w13">
      ...
      staff="{{encrypt($staff->staffId)}}">
        <option value="1" @if($staff->rtwProgress==1) selected="selected" @endif>To Be Started</option>
        <option value="2" @if($staff->rtwProgress==2) selected="selected" @endif>In Progress</option>
        <option value="3" @if($staff->rtwProgress==3) selected="selected" @endif>Completed</option>
      </select>
    </div>
...
<div class="row">
  <div class="col-md-12">
    ...
    <div class="box box-primary">
      <!-- form start -->
      <form action="{{route('staffs.right.to.work.update')}}" method="post" accept="application/pdf, image/*" enctype="multipart/form-data">
        {!! csrf_field() !!}
        <input type="hidden" name="staffRightToWorkId" value="{{encrypt($rtw->staffRightToWorkId)}}">
        <div class="box-body formRtW" visatype="{{$rtw->country->type}}">
          ...
          <div class='row'>
            ...
            <div class='col-sm-2 pull-right'>
              <label>&nbsp;</label>
              <div class='form-group'>
                <a href="{{asset('storage/app/staff/staff_rtw/passport/'.$rtw->passportDocumentFile)}}" target="_blank" class="btn btn-warning">View Passport</a>
              </div>
            </div>
            @endif
          </div>
          ...
          <div class='row visa @if($rtw->country->type==1) hidden @endif'>
            ...
            <div class='col-sm-2 pull-right'>
              <label>&nbsp;</label>
              <div class='form-group'>
                <a href="{{asset('storage/app/staff/staff_rtw/visa/'.$rtw->visaDocumentFile)}}" target="_blank" class="btn btn-warning">View Visa</a>
              </div>
            </div>
            @endif
          </div>
          ...
          <div class='row'>
            ...
            <div class="col-sm-4">
              <div class="form-group">
                  ...
                  <input class="form-control" name="checkedDate" autocomplete="off" type="text" value="{{date('d-m-Y',strtotime($rtw->checkedDate))}}"/>
                  <p class="error">Address is required</p>
              </div>
            </div>
          </div>
...
<script src="{{asset('public/js/pages/staffs/rtw/new.js')}}"></script>
@endpush
@push('css')
