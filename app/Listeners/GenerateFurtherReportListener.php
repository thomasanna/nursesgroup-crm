...
class GenerateFurtherReportListener
{
    /**
     ...
     * Handle the event.
     *
     * @param  GenerateFurtherReport  $event
     * @return void
     */
    public function handle(GenerateFurtherReport $event)
    {
      ...
      (new FastExcel($bookingList))->download('Payee_Report_'.date('d-M-Y',strtotime("-1 days")).".xlsx", function ($booking) {
          ...
          if($booking->requestedDate==NULL) $bookingDate = 'NA'; else date('d-m-Y',strtotime($booking->requestedDate));
          if($booking->importantNotes==NULL) $notes = 'NA'; else $notes = $booking->importantNotes;
          return [
              ...
              'Handled By'    => Admin::find($booking->handledBy)->name,
              'Requested By'    =>ClientUnitContact::find($booking->requestedBy)->fullName,
